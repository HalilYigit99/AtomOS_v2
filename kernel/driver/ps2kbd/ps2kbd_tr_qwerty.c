...
void __ps2kbd_tr_qwerty_handle(uint8_t scancode) {
    ...
    if (isExtended) {
        ...
        // Numbers row (TR QWERTY layout)
        case 0x0E: // " (double quote)
            data.ascii = '"';
            data.key = KEY_APOSTROPHE; // Using apostrophe key enum for quote
            data.isPressed = isPressed;
            data.upperCase = false;
            break;
        ...
        case 0x4E: // * (asterisk)
            data.ascii = '*';
            data.key = KEY_UNKNOWN; // No direct enum for asterisk
            data.isPressed = isPressed;
            data.upperCase = false;
            break;
